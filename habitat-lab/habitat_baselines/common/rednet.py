...
from habitat_baselines.rl.ppo.utils.constants import coco_categories_mapping, get_fourty_dict, mp3d_habitat_labels, hm3d_habitat_labels
...
class SemanticPredRedNet():
    ...
    def __init__(self, args):
        self.segmentation_model = load_rednet(args.ENVIRONMENT.sem_gpu_id,ckpt = args.ENVIRONMENT.checkpt, resize = True)
    ...
    def get_prediction(self, img, depth, cat_goal):
        ...
        img = torch.from_numpy(img).float().to(self.args.ENVIRONMENT.sem_gpu_id)
        depth = torch.from_numpy(depth).float().to(self.args.ENVIRONMENT.sem_gpu_id)
        #print(img.shape)
        #print(depth.shape)
