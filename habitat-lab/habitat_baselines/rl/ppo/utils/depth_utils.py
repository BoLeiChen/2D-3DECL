...
import habitat_baselines.rl.ppo.utils.rotation_utils as ru
...
def transform_pose_t(XYZ, current_pose, device):
    ...
    R = ru.get_r_matrix([0., 0., 1.], angle=current_pose[2] - np.pi / 2.)
    # print("R2",R)
    # print("R2--", torch.from_numpy(R).float().transpose(1, 0))
...
def transform_pose_t2(XYZ, current_pose, device):
    """
    Transforms the point cloud into geocentric frame to account for
    camera position
    Input:
